      ...
      RegisterCommand(command.Command, function(source, args, rawCommand)
        ...
        if hasPermissions then
          ...
          if _source ~= 0 then
            ...
            local s1, s2, s3 = args[1] and tostring(args[1]) or "", args[2] and tostring(args[2]) or "", args[3] and tostring(args[3]) or ""
            InsertHistoryAction(_source, string.format(Locales['COMMAND_HISTORY_ACTION_DESCRIPTION'], '<span style="color: gray;">' .. command.Command .. ' ' .. s1 .. ' ' .. s2 .. ' ' .. s3 .. '</span>'))
          end
...
AddEventHandler("tpz_admin:server:addChatSuggestions", function()
  ...
  if Config.CreateTicket.Enabled then 
    TriggerClientEvent("chat:addSuggestion", _source, "/" .. Config.CreateTicket.Command, Config.CreateTicket.Suggestion, {} )
  end
