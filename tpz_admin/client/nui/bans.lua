...
local RequestBans = function(selectedPage)

    if HAS_COOLDOWN then return end 

    HAS_COOLDOWN = true

    SendNUIMessage({ action = 'RESET_BANS_LIST' })

    local hasPermission = HasNUIPermissionByName("bans")
    ...
    TriggerEvent("tpz_core:ExecuteServerCallBack", "tpz_admin:getBannedUsers", function(result)
        ...
        if TPZ.GetTableLength(result) > 0 then
            ...
            for _, data in pairs (result) do 
                ...
                if (currentPage == selectedPage) then
                    ...
                    local converted_time  = convertSecondsToText(data.remaining_time)
                    local durationDisplay = data.remaining_time ~= -1 and Locales['BAN_VALID_UNTIL'] .. ' ' .. converted_time or Locales['PERMANENT_BAN']
                    data.remaining_time = durationDisplay

                    data.banned_reason_title = Locales['BANS_REASON_TITLE']
                    SendNUIMessage({
                        action = 'BANS_LIST_INSERT',
                        result = data,
...
RegisterNetEvent("tpz_admin:client:reload_bans")
AddEventHandler("tpz_admin:client:reload_bans", function()
    SEARCH_INPUT = nil
    RequestBans(1)
end)
...
RegisterNUICallback('request_bans_section', function()
    SEARCH_INPUT = nil
    RequestBans(1)
end)

-- @data.page
RegisterNUICallback('request_bans_section_page', function(data)
    RequestBans(tonumber(data.page))
end)

-- @data.input 
RegisterNUICallback('request_bans_section_search_input', function(data)
...
RegisterNUICallback('unban', function(data)
    TriggerServerEvent("tpz_admin:server:unban", data.steamname, data.identifier)
end)
